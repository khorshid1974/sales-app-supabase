<template>
    <div class="m-4 p-4">
      ...
      <div class="flex flex-row justify-between items-center border p-1">  
        ...
        <button class="p-2 rounded-md bg-slate-200 border" type="button" @close="close" @click="()=> isOpen = !isOpen">show items</button>
        <ul class="flex flex-row  text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            <li class="w-full border-b border-gray-200 rounded-t-lg dark:border-gray-600">
                <div class="flex items-center ps-3">
                    <input @change="changeTypeToCash($event)"  id="list-radio-license" type="radio" checked v-model="paymentType" value="1" name="list-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                    <label for="list-radio-license" class="w-full py-3 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300"> Cash </label>
                </div>
            </li>
            <li class="w-full border-b border-gray-200 rounded-t-lg dark:border-gray-600">
                <div class="flex items-center ps-3">
                    <input @change="changeTypeToLater($event)"  id="list-radio-id" type="radio"  v-model="paymentType" value="2" name="list-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                    <label for="list-radio-id" class="w-full py-3 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Later</label>
                </div>
            </li>
            <li class="w-full border-b border-gray-200 rounded-t-lg dark:border-gray-600">
                <div class="flex items-center ps-3">
                    <input @change="changeTypeToInstallment" id="list-radio-military" type="radio"  v-model="paymentType" value="3" name="list-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                    <label for="list-radio-military" class="w-full py-3 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Installment</label>
                </div>
            </li>
          ...
          <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                v-for="item in tableData" :key="item.id">
                    ...
                    <td class="px-6 py-4">{{ item.SalePrice }}</td>
                    <td class="px-6 py-4">
                      <button class=" px-2 rounded-md bg-red-200" type="button" @click="()=>item.Quantity>1? item.Quantity-- : item.Quantity">-</button>
                      {{ item.Quantity }}
                      <button class="px-2 rounded-md bg-green-200" type="button" @click="()=> item.Quantity++">+</button>
                    </td>
                    <td class="px-6 py-4">{{ item.Quantity * item.SalePrice }}</td>
       ...
       <div class="flex flex-col justify-end border-gray-500 space-y-2 mt-3">
          <div class="flex flex-row justify-end space-x-2">
            ...
            <input class="border rounded-md p-2" type="number" id="Amount" name="Amount" placeholder="Amount" v-model="sum" disabled>
            <label for="payment" class="text-lg text-yellow-800 border rounded-md p-2">Payment</label>
            <input class="border rounded-md p-2" type="number" disabled  id="payment" name="payment" placeholder="Payment" v-model="payment">
...
const newListOfItems = searchResults.value.map(item => ({ InvoiceId: item.InvoiceId, Itemid: item.ItemId, Quantity: item.Quantity,  ItemName: item.Name, SalePrice: item.SalePrice, PurchasePrice: item.PurchasePrice }));
...
const changeTypeToCash = (event) => {
  payment.value= sum.value;
}
const changeTypeToLater = (event) => {
  payment.value= 0;
}
const sum= computed(() => {
